//// Basic VLM Installation ////
import VLM, { QuickCreator } from 'vlm-dcl'
import {
  Entity,
  InputAction,
  Material,
...
export let modelMundo: Entity = engine.addEntity()
export let modelBuilding: Entity = engine.addEntity()
...
export async function main() {
  ...
  // // create system
  // engine.addSystem(() => {
  //   if (inputSystem.isTriggered(InputAction.IA_POINTER, PointerEventType.PET_DOWN, meshEntity)) {
  //     // Logic in response to button press on myEntity
  //     const transform = Transform.getMutable(modelMundo)
  //     transform.scale.x = transform.scale.x * 1.1
  //     transform.scale.y = transform.scale.y * 1.1
  //     transform.scale.z = transform.scale.z * 1.1
  //     transform.position.y -= 0.5
  //   }
  // })
  modelMundo = new QuickCreator.Mesh({
    path: 'mundo.glb',
    position: { x: 18, y: 8, z: 24 },
    scale: { x: 3, y: 3, z: 3 }
  }).entity

  modelBuilding = new QuickCreator.Mesh({
    path: 'alfajor.glb',
