...
export const mate = engine.addEntity()

export function createBeerGlass(model: string, position: Vector3, id: SyncEntityIDs) {
  // const glassEntity = engine.addEntity()

  GltfContainer.create(mate, {
    src: model,
    visibleMeshesCollisionMask: ColliderLayer.CL_POINTER,
    invisibleMeshesCollisionMask: undefined
  })

  Transform.create(mate, { position, rotation: Quaternion.fromEulerDegrees(0, 0,0), scale: Vector3.create(1.5, 1.5, 1.5) })

  Animator.create(mate, {
    states: [
      {
        clip: 'Blank',
        playing: true
  ...
  BeerGlass.create(mate)

  PointerEvents.create(mate, {
    pointerEvents: [
      {
